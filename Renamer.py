	...
	def scan_folder(self):
		...
		try:
			...
			try:
				wb = Workbook()
				ws = wb.active
				ws.append(['File Name', 'New File Name', 'Extension', "{}".format(self.folder)])
				...
				for i in files:
					...
					if os.path.isfile("{}/{}".format(self.folder, i)):
						...
						ws.append([filename, '', file_extension])

				wb.save(name_file)

			except IOError:
				showwarning(title="File inaccessible", message="Please close names.xlsx")
